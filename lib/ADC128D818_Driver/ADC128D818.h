...
public:
  ...
  uint16_t read(uint8_t channel, bool useRTOSDelay = true);

  double readConverted(uint8_t channel, bool useRTOSDelay = true);

  double readTemperatureConverted(bool useRTOSDelay = true);
...
private:
  ...
  uint8_t readCurrentRegister8(bool useRTOSDelay = true);
};
